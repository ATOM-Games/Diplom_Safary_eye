...
class TextConverter(object):
    def store_in_dct(self, lst: List, dct: Dict):
        ...
        :param dct: destination dict
        '''
        for str in lst:
            if re.match(r'^.*=.*', str):  # if including '=':
                spl = str.split('=')
                dct[spl[0]] = spl[1]
    ...
    def convert(self, file_name: str) -> List:
        ...
        with open(file_name, 'r', encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if not row:
                    continue
                if re.match(r'^\s*#', row[0]):  # if initial char is "#":
                    continue
                try:
...
class Txt2Yaml(TextConverter):
    def generate(self, input_file: str, output_file: str):
        ...
        if not re.search(r'\.(YAML|YML)$', output_file.upper()):
            raise NameError("Extension of output filename should be yaml or yml."
...
def parse_option():
    dc = 'Convert style text file to yaml file.'
    parser = argparse.ArgumentParser(description=dc, prog='text2yaml',
