...
papka = 'Models/last_my/'

device = select_device('cpu')
model = DetectMultiBackend( weights='Models/last_my/best.pt', device=device, dnn=False, data = 'Models/last_my/ABC_weapon_02.yaml')
...
def DetMy(img, s1, s2):
    im0 = img
    img = letterbox(im0, (480, 480), stride=32)[0]
    ...
    pred = non_max_suppression(pred, s1, 0.45, None, False, max_det=1000)
    str_str = []
    for i, det in enumerate(pred):
        annotator = Annotator(im0, line_width=3, example=str(names))
        if len(det):
